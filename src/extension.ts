...
export function activate(context: vscode.ExtensionContext) {
    ...
    // 查找匹配的括号位置
    function findMatchingBracket(document: vscode.TextDocument, position: vscode.Position): vscode.Position | null {
        ...
        else if (char === '}') { endChar = '{'; isForward = false; }
        else if (char === ']') { endChar = '['; isForward = false; }
        else return null;
        ...
        if (isForward) {
            while (++currentOffset < text.length) {
                if (text[currentOffset] === startChar) count++;
    ...
    // 获取括号位置（包括周围空格）
    function getBracketPosition(document: vscode.TextDocument, position: vscode.Position): vscode.Position | null {
            ...
            if (bracketPosition) {
                ...
                if (matchingPosition) {
                    ...
                    const selection = new vscode.Selection(
                        startPos.translate(0, 1), // 从开始括号后一个字符开始
                        endPos // 到结束括号
                    );
